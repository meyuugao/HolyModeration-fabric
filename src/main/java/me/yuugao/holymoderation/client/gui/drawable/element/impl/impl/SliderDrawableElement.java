...
public class SliderDrawableElement extends DrawableElement {
    ...
    protected void render(DrawContext ctx, int z) {
        ...
        render2DService.renderSoftRoundedRectOutline(ctx, 0f, trackY, getWidth(), TRACK_HEIGHT, z,
                radius, trackColor, outlineColor, outlineWidth, blurWidth);
        if (fillWidth > 1f) {
            render2DService.renderSoftRoundedRect(ctx, 0f, trackY, Math.min(fillWidth, getWidth()), TRACK_HEIGHT, z,
                    radius, fillColor, (int) blurWidth);
        }

        float knobX = getWidth() * f - KNOB_RADIUS;
        float knobY = getHeight() / 2f - KNOB_RADIUS;
        render2DService.renderSoftRoundedRectOutline(ctx, knobX, knobY, KNOB_RADIUS * 2f, KNOB_RADIUS * 2f, z,
