...
public class PopupColorEditor {
    ...
    public void render(DrawContext ctx, int z, ThemePalette palette, float pW, float pH) {
        ...
        render2DService.renderSoftRoundedRect(ctx, px, py, panelW, panelH, z + 1, 12f, palette.surfaceElevated, 0);
        render2DService.renderSoftRoundedRectOutline(ctx, px, py, panelW, panelH, z + 1, 12f,
                palette.surfaceElevated, palette.outline, 1.5f, 2f);

        cbX = px + panelW - CB_SIZE - 8f;
        cbY = py + 8f;
        render2DService.renderSoftRoundedRect(ctx, cbX, cbY, CB_SIZE, CB_SIZE, z + 2, 6f, palette.surface, 0);
        String cross = "✕";
