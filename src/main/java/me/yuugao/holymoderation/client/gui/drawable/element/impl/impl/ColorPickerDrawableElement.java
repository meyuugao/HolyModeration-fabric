...
public class ColorPickerDrawableElement extends DrawableElement {
    ...
    protected void render(DrawContext ctx, int z) {
        ...
        render2DService.renderSVSquare(ctx, 0f, 0f, squareSize, squareSize, z, cornerRadius, hue, outlineColor, outlineWidth);
        render2DService.renderHueBar(ctx, squareSize + HUE_GAP, 0f, barWidth, squareSize, z, cornerRadius, outlineColor, outlineWidth);

        if (selectedColor != null) {
            renderSquareIndicator(ctx, z);
            renderHueIndicator(ctx, z);
        }

        render2DService.endRender();
    }

    private void renderSquareIndicator(DrawContext ctx, int z) {
    ...
    private void renderHueIndicator(DrawContext ctx, int z) {
        float y = hue * squareSize;
        float h = Math.max(2f, squareSize * 0.025f);
        float x = squareSize + HUE_GAP - 1.5f;

        render2DService.renderSoftRoundedRectOutline(ctx, x, y - h / 2f, barWidth + 3f, h, z,
