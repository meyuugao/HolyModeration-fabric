...
public class AppearanceTab extends SettingsTab {
    ...
    protected void renderExtra(DrawContext ctx, ThemePalette palette, float pW, float pH, int z, float scroll) {
        ...
        for (ElementColorRow row : elementRows) {
            ...
            render2DService.renderSoftRoundedRect(ctx.getMatrices(), row.swatchX, row.swatchY, SWATCH_SIZE, SWATCH_SIZE, z,
                    5f, color, 0);
            render2DService.renderSoftRoundedRectOutline(ctx.getMatrices(), row.swatchX, row.swatchY, SWATCH_SIZE, SWATCH_SIZE, z,
                    5f, color, palette.outline, 1.2f, 1f);
            render2DService.renderSoftRoundedRect(ctx.getMatrices(), row.swatch2X, row.swatch2Y, SWATCH_SIZE, SWATCH_SIZE, z,
                    5f, color2, 0);
            render2DService.renderSoftRoundedRectOutline(ctx.getMatrices(), row.swatch2X, row.swatch2Y, SWATCH_SIZE, SWATCH_SIZE, z,
