...
public class NetModule {
    ...
    private void refresh() {
        ...
        if (apiConfig.getApiToken().isEmpty()) {
        ...
        netService.downloadSounds().thenRun(() -> {
            try {
                ...
                if (journalProfile == null || journalStats == null
                        || journalProfile.equals(Collections.emptyMap()) || journalStats.equals(Collections.emptyMap())) {
                    modStateService.setOnlineMode(false);
                    notificationsService.addNotification(NotificationType.ERROR, "%s%sОшибка".formatted(RED, BOLD),
                ...
                Object idVkObj = journalProfile.get("idVk");
                Object nicknameObj = journalProfile.get("nickname");
                if (idVkObj == null || nicknameObj == null) {
                    modStateService.setOnlineMode(false);
                    notificationsService.addNotification(NotificationType.ERROR, "%s%sОшибка".formatted(RED, BOLD),
                            "Получен некорректный ответ от сервера журнала. Попробуйте перезайти позже.", 10f);
                    return;
                }

                modStateService.setOnlineMode(true);

                String vk = "vk.com/id%s".formatted((long) Double.parseDouble(idVkObj.toString()));
                apiConfig.setVk(vk);
                ...
                // Приоритет: несовпадение ника — полная блокировка (выше чем устаревшая версия)
                if (!userStateService.getUserNickname().equals(nicknameObj.toString())) {
                    notificationsService.addNotification(NotificationType.ERROR, "%s%sОшибка".formatted(RED, BOLD),
                            "Ваш никнейм не совпадает с никнеймом из журнала. Мод был полностью заблокирован. Пожалуйста, используйте свой API-ключ.", 3600f);
                ...
                // Ник совпал — теперь проверяем обновление (частичная блокировка, Логика X)
                if (needUpdates()) return;

                if (modStateService.isUpdateBlocked()) {
                    modStateService.unblockForUpdate();
                }
            } catch (Exception e) {
                modStateService.setOnlineMode(false);
                notificationsService.addNotification(NotificationType.ERROR, "%s%sОшибка".formatted(RED, BOLD),
                        "Не удалось выполнить синхронизацию с журналом: %s".formatted(e.getMessage()), 10f);
            }
        });
    }
    ...
    private boolean needUpdates() {
        ApiConfig apiConfig = configManagerService.getApiConfig();
        AbstractMap.SimpleEntry<String, String> lastUpdates;
        try {
            lastUpdates = netService.getLastUpdates().get();
        } catch (ExecutionException | InterruptedException e) {
            notificationsService.addNotification(NotificationType.ERROR, "%s%sОшибка".formatted(RED, BOLD),
                    "Не удалось проверить обновления. Проверьте подключение к интернету и попробуйте позже.", 7f);
            return false;
        }

        // Сеть недоступна или файл LATEST.txt недоступен — не крашим, продолжаем работу.
        if (lastUpdates == null) {
            return false;
        }
        ...
        if (!apiConfig.getCurrentVersion().equals(lastVersion)) {
            ...
            modStateService.blockForUpdate();

            // При ограниченном режиме выключаем все авто-функции, кроме автобана.
            SettingsConfig settingsConfig = configManagerService.getSettingsConfig();
            settingsConfig.setAutoVanishEnabled(false);
            settingsConfig.setAutoAnyDeskEnabled(false);
            settingsConfig.setDupeIpEnabled(false);
            settingsConfig.setAutoFlyEnabled(false);
            settingsConfig.setAutoGm3Enabled(false);
            settingsConfig.setAutoGodEnabled(false);
            settingsConfig.setAutoHacAlertsEnabled(false);
            settingsConfig.setAutoCheckoutTpEnabled(false);
            settingsConfig.setAutoSpyTpEnabled(false);
