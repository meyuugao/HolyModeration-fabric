...
public class StringEncryptionModule {
    ...
    public static void obfuscateMethods(ClassNode cn, MethodNode mn, String decoderName) {
        if ((cn.access & Opcodes.ACC_ANNOTATION) != 0) return;
        if (mn.instructions == null) return;
        if (mn.name.equals(decoderName)) return;
    ...
    public static void obfuscateFields(ClassNode cn, String decoderName) {
    ...
    private static InsnList buildObfuscatedInsn(String owner, String decoder, byte[] enc, int key) {
        ...
        // Вызываем вспомогательный мутирующий метод
    ...
    private static MethodNode getOrCreateInit(ClassNode cn, boolean isStatic) {
        ...
        MethodNode m = new MethodNode(
                isStatic ? Opcodes.ACC_STATIC : Opcodes.ACC_PUBLIC,
                name, "()V", null, null
        );
