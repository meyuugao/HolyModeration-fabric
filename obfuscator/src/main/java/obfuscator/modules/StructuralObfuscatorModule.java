...
public class StructuralObfuscatorModule {
    ...
    private static void injectFakeLocals(MethodNode method) {
        ...
        method.instructions.add(end);
        for (int i = 0; i < 2; i++) {
            String fakeName = "fakeVar" + Math.abs(new java.util.Random().nextInt());
    ...
    private static void duplicateMethods(ClassNode cn) {
        for (MethodNode method : cn.methods.toArray(new MethodNode[0])) {
            ...
            MethodNode dup = new MethodNode(org.objectweb.asm.Opcodes.ASM9, method.access, NameGenerator.generateChineseName(), method.desc, method.signature, method.exceptions.toArray(new String[0]));
